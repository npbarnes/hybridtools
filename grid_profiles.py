...
def profile_func(grid_points, grid_data):
    rgi = RegularGridInterpolator(points=grid_points, values=grid_data, bounds_error=False, fill_value=0.0)
    return rgi

def profile(interp_points, grid_points, grid_data):
    rgi = profile_func(grid_points, grid_data)
...
def direct_profiler_func_factory(hf, var, modifier=None, step=-1):
    hybrid = hr(hf, var)
    grid_points = hybrid.para['grid_points']
    v = hybrid.get_timestep(step)[-1]
    if modifier is not None:
    ...
    return profile_func(grid_points, data), grid_points
...
vmag_profiler = direct_profiler_factory('up', modifier=vector_mag)
...
vmag_profiler_func, grid = direct_profiler_func_factory(shell_prefixes[2], 'up', modifier=vector_mag)
density_profiler_func, _ = direct_profiler_func_factory(shell_prefixes[2], 'np', modifier=n_km2cm)
sw_profiler_func, _ = direct_profiler_func_factory(shell_prefixes[2], 'np_He', modifier=n_km2cm)
